...
// ── Differentials panel: Top-3 conditions with confidence, collapsible ──────
function DifferentialsPanel({ diffs, lang }: { diffs: Differential[]; lang: Lang }) {
  const [open, setOpen] = useState(false);
  if (!diffs || diffs.length < 2) return null;
  const others = diffs.slice(1, 3);
  return (
    <div style={{ marginTop: "0.875rem", borderTop: "1px dashed #E5E7EB", paddingTop: "0.875rem" }}>
      <button
        ...
        <span style={{ fontSize: "0.78rem", fontWeight: 600, color: "#1A56DB" }}>
          {lang === "bm" ? "Lihat kemungkinan lain" : "See other possibilities"}
          <span style={{ fontSize: "0.68rem", color: "#9CA3AF", fontWeight: 500, marginLeft: "0.4rem" }}>({others.length})</span>
        </span>
        <span style={{ fontSize: "0.7rem", color: "#6B7280", transition: "transform 0.2s", transform: open ? "rotate(180deg)" : "none" }}>▼</span>
      </button>
      {open && (
        <div style={{ marginTop: "0.875rem", display: "flex", flexDirection: "column", gap: "0.625rem" }}>
          <div style={{ fontSize: "0.7rem", color: "#6B7280", lineHeight: 1.5 }}>
            {lang === "bm"
              ? "AI juga mempertimbangkan kemungkinan ini. Doktor akan menyemak."
              : "The AI also considered these. A doctor will review."}
          </div>
          {others.map((d, i) => {
            const priColor = d.priority === "P1" ? "#E02424" : d.priority === "P2" ? "#1A56DB" : "#065F46";
            const priBg    = d.priority === "P1" ? "#FEE2E2" : d.priority === "P2" ? "#DBEAFE" : "#D1FAE5";
            return (
              <div key={i} style={{ background: "#F9FAFB", borderRadius: "0.5rem", padding: "0.625rem 0.75rem" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.4rem", gap: "0.5rem" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "0.4rem", minWidth: 0, flex: 1 }}>
                    <span style={{ fontSize: "0.6rem", background: priBg, color: priColor, padding: "0.1rem 0.4rem", borderRadius: 9999, fontWeight: 700, flexShrink: 0 }}>{d.priority}</span>
                ...
                <div style={{ fontSize: "0.7rem", color: "#6B7280", marginBottom: "0.35rem" }}>{d.department}</div>
                <div style={{ height: 4, borderRadius: 2, background: "#E5E7EB", overflow: "hidden" }}>
                  <div style={{ height: "100%", width: `${d.confidence}%`, background: priColor, borderRadius: 2, transition: "width 0.5s ease" }} />
                </div>
              </div>
            );
          })}
...
export default function TriagePage() {
  ...
  const [reviewStatus, setReviewStatus] = useState<"PENDING_REVIEW" | "REVIEWED" | "AUTO_APPROVED" | null>(null);
  const [reviewerInfo, setReviewerInfo] = useState<{ name: string; at: number; notes?: string; overridden: boolean; new_priority?: Priority; new_dept?: string; new_doctor?: string } | null>(null);
  ...
  // ── HITL polling: P2 cases wait for doctor review ─────────────────────────
  useEffect(() => {
    if (!sessionId || reviewStatus !== "PENDING_REVIEW") return;
    let cancelled = false;
    const tick = async () => {
      try {
        const res = await fetch(`/api/cases/${sessionId}/status`);
        if (!res.ok) return;
        ...
        if (data.review_status === "REVIEWED") {
          ...
          setReviewerInfo({
            name:        data.reviewed_by ?? "Doctor",
            at:          data.reviewed_at ?? Date.now(),
            notes:       data.review_notes,
            overridden:  data.overridden,
            new_priority: data.overridden ? data.priority : undefined,
            new_dept:    data.overridden ? data.department : undefined,
            new_doctor:  data.overridden ? data.doctor_name : undefined,
          });
          // If doctor changed priority, update the result so result-render uses it
          if (data.overridden && data.priority && result) {
            setResult({ ...result, priority: data.priority, department: data.department, assigned_doctor: result.assigned_doctor ? { ...result.assigned_doctor, name: data.doctor_name, specialty: data.department } : result.assigned_doctor });
          }
        }
  ...
  async function runTriage() {
    ...
    try {
      ...
      setSessionId(sid);
      setReviewStatus(tData.priority === "P2" ? "PENDING_REVIEW" : "AUTO_APPROVED");
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "#F1F5F9" }}>
      ...
      <main className="page-main" style={{ display: "flex", flexDirection: "column", gap: "0" }}>
        ...
        {/* ── P1 EMERGENCY: Call 999 immediately, hospital pre-notified ── */}
        {cardStep === "result" && result && result.priority === "P1" && (
          <div style={{ maxWidth: 640, margin: "0 auto", display: "flex", flexDirection: "column", gap: "1rem" }}>
            <div style={{
              ...
              color: "#fff", borderRadius: "1rem", padding: "1.75rem",
              textAlign: "center", boxShadow: "0 12px 32px rgba(224,36,36,0.25)",
              border: "2px solid #fff",
            }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "0.5rem", marginBottom: "0.75rem" }}>
                <RiAlarmWarningFill size={26} style={{ animation: "pulse 1s ease-in-out infinite" }} />
                <span style={{ fontSize: "0.78rem", fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase" }}>
                  {lang === "bm" ? "Kecemasan Dikesan" : "Emergency Detected"}
                </span>
              </div>
              <h2 className="font-heading" style={{ fontSize: "1.65rem", color: "#fff", marginBottom: "0.5rem", lineHeight: 1.25 }}>
                {lang === "bm" ? "Hubungi 999 Sekarang" : "Call 999 Right Now"}
              </h2>
              <p style={{ fontSize: "0.92rem", color: "rgba(255,255,255,0.92)", lineHeight: 1.6, marginBottom: "1.5rem", maxWidth: 460, margin: "0 auto 1.5rem" }}>
                {lang === "bm"
                  ? "Gejala anda menunjukkan kecemasan perubatan. Jangan tunggu — hubungi 999 untuk ambulans segera."
                  : "Your symptoms indicate a medical emergency. Don't wait — call 999 for an ambulance immediately."}
              </p>
              <a href="tel:999"
                style={{
                  display: "inline-flex", alignItems: "center", gap: "0.625rem",
                  background: "#fff", color: "#E02424",
                  padding: "1rem 2rem", borderRadius: "0.75rem",
                  fontWeight: 800, fontSize: "1.15rem", textDecoration: "none",
                  boxShadow: "0 4px 16px rgba(0,0,0,0.2)",
                }}>
                <RiPhoneLine size={22} />
                {lang === "bm" ? "Telefon 999" : "Dial 999"}
              </a>
            </div>

            {/* Hospital pre-notification confirmation */}
            {result.recommended && (
              <div className="card" style={{ borderLeft: "4px solid #059669", padding: "1.25rem 1.5rem" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.4rem" }}>
                  <RiCheckLine size={18} color="#059669" />
                  <span style={{ fontSize: "0.78rem", fontWeight: 700, color: "#059669", textTransform: "uppercase", letterSpacing: "0.06em" }}>
                    {lang === "bm" ? "Hospital Telah Dimaklumkan" : "Hospital Pre-Notified"}
                  </span>
                </div>
                ...
                <div style={{ fontSize: "0.82rem", color: "#6B7280", lineHeight: 1.6 }}>
                  {lang === "bm"
                    ? "Pasukan kecemasan di hospital ini telah dimaklumkan tentang gejala dan ketibaan anda. Mereka akan bersedia."
                    : "The emergency team at this hospital has been notified of your symptoms and incoming arrival. They'll be ready."}
                </div>
                {result.recommended.phone && (
                  <a href={`tel:${result.recommended.phone}`} style={{ display: "inline-flex", alignItems: "center", gap: "0.4rem", marginTop: "0.625rem", color: "#1A56DB", fontWeight: 600, fontSize: "0.85rem", textDecoration: "none" }}>
                    <RiPhoneLine size={14} />{result.recommended.phone}
                  </a>
                )}
              </div>
            )}
            ...
            <div className="card" style={{ padding: "1.25rem 1.5rem" }}>
              <div style={{ fontSize: "0.78rem", fontWeight: 700, color: "#111827", marginBottom: "0.625rem" }}>
                {lang === "bm" ? "Maklumkan kepada operator 999:" : "Tell the 999 operator:"}
              </div>
              <ul style={{ listStyle: "disc", paddingLeft: "1.25rem", display: "flex", flexDirection: "column", gap: "0.4rem", margin: 0 }}>
                <li style={{ fontSize: "0.85rem", color: "#374151" }}>
                  <strong>{lang === "bm" ? "Lokasi anda:" : "Your location:"}</strong> {locLabel}
                </li>
                <li style={{ fontSize: "0.85rem", color: "#374151" }}>
                  <strong>{lang === "bm" ? "Gejala utama:" : "Main symptoms:"}</strong> {result.key_symptoms?.slice(0, 3).join(", ") ?? "—"}
                </li>
                {painScore > 0 && (
                  <li style={{ fontSize: "0.85rem", color: "#374151" }}>
                    <strong>{lang === "bm" ? "Skor sakit:" : "Pain score:"}</strong> {painScore}/10
                  </li>
                )}
              </ul>
            </div>

            <button className="btn-outline" onClick={reset} style={{ width: "fit-content", display: "flex", alignItems: "center", gap: "0.4rem", margin: "0 auto" }}>
              <RiArrowLeftLine size={15} /> {lang === "bm" ? "Penilaian Baru" : "New Assessment"}
            </button>
          </div>
        )}

        {/* ── P2 AWAITING DOCTOR REVIEW ── */}
        {cardStep === "result" && result && result.priority === "P2" && reviewStatus === "PENDING_REVIEW" && (
          <div style={{ maxWidth: 560, margin: "0 auto", display: "flex", flexDirection: "column", gap: "1rem" }}>
            <div className="card" style={{ textAlign: "center", padding: "2.5rem 2rem" }}>
              <div style={{ width: 64, height: 64, borderRadius: "50%", background: "#EFF6FF", border: "2px solid #1A56DB", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 1.25rem", animation: "pulse 1.5s ease-in-out infinite" }}>
                <RiUserSearchLine size={28} color="#1A56DB" />
              </div>
              <div style={{ fontSize: "0.7rem", fontWeight: 700, color: "#1A56DB", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: "0.5rem" }}>
                {lang === "bm" ? "Menunggu Pengesahan Doktor" : "Awaiting Doctor Review"}
              </div>
              <h2 style={{ fontSize: "1.25rem", fontWeight: 700, color: "#111827", marginBottom: "0.625rem" }}>
                {lang === "bm" ? "Doktor sedang menyemak penilaian anda" : "A doctor is reviewing your assessment"}
              </h2>
              <p style={{ fontSize: "0.875rem", color: "#6B7280", lineHeight: 1.65, marginBottom: "1.5rem", maxWidth: 400, margin: "0 auto 1.5rem" }}>
                {lang === "bm"
                  ? "Untuk kes mendesak, doktor sebenar mengesahkan keputusan AI sebelum anda dirujuk. Ini biasanya mengambil masa 1–3 minit."
                  : "For urgent cases, a real doctor confirms the AI's decision before you're directed. This usually takes 1–3 minutes."}
              </p>
              <div style={{ display: "inline-flex", alignItems: "center", gap: "0.5rem", background: "#F0FDF4", border: "1px solid #86EFAC", borderRadius: 9999, padding: "0.4rem 0.875rem", fontSize: "0.78rem", fontWeight: 600, color: "#059669" }}>
                <span style={{ width: 7, height: 7, borderRadius: "50%", background: "#059669", animation: "pulse 1s ease-in-out infinite" }} />
                {lang === "bm" ? "AI provisional: P2 Segera" : "AI provisional: P2 Urgent"}
              </div>
            </div>
            ...
            <div className="card" style={{ padding: "1rem 1.25rem", display: "flex", alignItems: "flex-start", gap: "0.625rem" }}>
              ...
              <div style={{ fontSize: "0.78rem", color: "#374151", lineHeight: 1.6 }}>
                {lang === "bm"
                  ? "Jika gejala anda bertambah teruk semasa menunggu, hubungi 999 segera. AI tidak menggantikan penilaian klinikal."
                  : "If your symptoms get worse while you wait, call 999 immediately. The AI does not replace clinical judgment."}
              </div>
            </div>

            <button className="btn-outline" onClick={reset} style={{ width: "fit-content", display: "flex", alignItems: "center", gap: "0.4rem", margin: "0 auto" }}>
              <RiArrowLeftLine size={15} /> {lang === "bm" ? "Batal" : "Cancel"}
            </button>
          </div>
        )}

        {/* ── RESULT (P3 always, P2 after review, P1 already handled above) ── */}
        {cardStep === "result" && result && pc && result.priority !== "P1" && !(result.priority === "P2" && reviewStatus === "PENDING_REVIEW") && (() => {
          ...
          return (
            <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
              ...
              {/* Doctor review badge — shown when a doctor confirmed/overrode the AI */}
              {reviewerInfo && (
                <div style={{
                  background: reviewerInfo.overridden ? "#FEF9C3" : "#F0FDF4",
                  border: `1.5px solid ${reviewerInfo.overridden ? "#FCD34D" : "#86EFAC"}`,
                  ...
                  <div style={{
                    ...
                    <RiUserSearchLine size={18} color={reviewerInfo.overridden ? "#92400E" : "#059669"} />
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: "0.7rem", fontWeight: 700, color: reviewerInfo.overridden ? "#92400E" : "#059669", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: "0.2rem" }}>
                      {reviewerInfo.overridden
                        ? (lang === "bm" ? "Dikemas Kini oleh Doktor" : "Updated by Doctor")
                        : (lang === "bm" ? "Disahkan oleh Doktor" : "Confirmed by Doctor")}
                    </div>
                    <div style={{ fontSize: "0.92rem", fontWeight: 700, color: "#111827", marginBottom: "0.25rem" }}>
                      {reviewerInfo.name}
                    </div>
                    {reviewerInfo.overridden && reviewerInfo.new_priority && (
                      <div style={{ fontSize: "0.8rem", color: "#374151", marginBottom: "0.2rem" }}>
                        {lang === "bm" ? "Keutamaan dikemas kini:" : "Priority updated:"} <strong>{reviewerInfo.new_priority}</strong>
                        {reviewerInfo.new_dept && <> · <strong>{reviewerInfo.new_dept}</strong></>}
                      </div>
                    )}
                    {reviewerInfo.notes && (
                      <div style={{ fontSize: "0.78rem", color: "#6B7280", lineHeight: 1.55, fontStyle: "italic", marginTop: "0.3rem" }}>
                        “{reviewerInfo.notes}”
                      </div>
                    )}
                  </div>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", flexWrap: "wrap" }}>
                      ...
                      {result.differentials?.[0]?.confidence !== undefined && (
                        <span style={{
                          ...
                          fontSize: "0.7rem", fontWeight: 700, color: pc.color,
                          background: pc.bg, padding: "0.15rem 0.5rem", borderRadius: 9999,
                        }}
                          title={lang === "bm" ? "Tahap keyakinan AI" : "AI confidence level"}>
